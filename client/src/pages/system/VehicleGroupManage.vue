<template>
  <div class="vehicle-group-container p-8 min-h-screen bg-gray-50 dark:bg-gray-900">
    <div class="page-header mb-8 flex justify-between items-center">
      <div>
        <h2 class="text-3xl font-bold text-gray-800 dark:text-gray-200">🚛 车辆组别管理</h2>
        <p class="text-gray-500 dark:text-gray-400 mt-1">管理主车分组类型</p>
      </div>
...
<script>
import { getVehicleGroups, createVehicleGroup, updateVehicleGroup, deleteVehicleGroup } from '@/api/vehicleGroups'
...
export default {
  ...
  methods: {
    async fetchData() {
      this.loading = true
      try {
        const res = await getVehicleGroups(this.query)
        this.list = res.data.list
        this.total = res.data.total
      } catch (error) {
        console.error('Fetch vehicle groups error:', error)
      } finally {
        this.loading = false
      }
    },
    ...
    async handleDelete(row) {
      try {
        ...
        await deleteVehicleGroup(row.id)
        ...
        if (valid) {
          try {
            if (this.dialogType === 'add') {
              await createVehicleGroup(this.form)
              this.$message.success('添加成功')
            } else {
              await updateVehicleGroup(this.form.id, this.form)
