<template>
  <div class="driver-group-container p-8 min-h-screen bg-gray-50 dark:bg-gray-900">
    <div class="page-header mb-8 flex justify-between items-center">
      <div>
        <h2 class="text-3xl font-bold text-gray-800 dark:text-gray-200">👥 司机组别管理</h2>
        <p class="text-gray-500 dark:text-gray-400 mt-1">管理司机分组类型</p>
      </div>
...
<script>
import { getDriverGroups, createDriverGroup, updateDriverGroup, deleteDriverGroup } from '@/api/driverGroups'
...
export default {
  ...
  methods: {
    async fetchData() {
      this.loading = true
      try {
        console.log('Fetching driver groups...')
        const res = await getDriverGroups(this.query)
        console.log('Driver groups response:', res)
        this.list = res.data.list
        this.total = res.data.total
      } catch (error) {
        console.error('Fetch driver groups error:', error)
      } finally {
        this.loading = false
      }
    },
    ...
    async handleDelete(row) {
      try {
        ...
        await deleteDriverGroup(row.id)
        ...
        if (valid) {
          try {
            if (this.dialogType === 'add') {
              await createDriverGroup(this.form)
              this.$message.success('添加成功')
            } else {
              await updateDriverGroup(this.form.id, this.form)
